	...
	for _, dat := range [][]byte{
		...
		b.Run("buffered-"+strconv.Itoa(len(dat)), func(b *testing.B) {
			...
			for n := 0; n < b.N; n++ {
				...
				resp = clservev3.NewBufferResponse(resp, -1)
				Expect(resp.Write(dat)).ToNot(BeZero())

				rbuf, _ := resp.(*clservev3.ResponseBuffer)
				rbuf.ImplicitFlush()
				rbuf.Free()
			}
		})
		...
		srv2 := &httptest.Server{
			...
			Config: &http.Server{
				ReadHeaderTimeout: time.Second, Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					defer GinkgoRecover()
					wr := clservev3.NewBufferResponse(w, 10)
					handler(wr, r)
...
var _ = Describe("buffered writes", func() {
	...
	Describe("limiting", func() {
		It("should limit writes exactly", func() {
			...
			wrt = clservev3.NewBufferResponse(rec, 1)
			...
			Expect(errors.Is(err, clservev3.ErrBufferFull)).To(BeTrue(), "should have been this error")
		...
		It("should limit writes when writing past", func() {
			...
			Expect(n).To(Equal(0))
			Expect(errors.Is(err, clservev3.ErrBufferFull)).To(BeTrue())
		...
		It("should flush correctly", func() {
			rec := httptest.NewRecorder()
			fwr = clservev3.NewBufferResponse(rec, 2)
	...
	It("should unwrap correctly", func() {
		rec := httptest.NewRecorder()
		fwr = clservev3.NewBufferResponse(rec, 0)
	...
	It("should pass on flush errors", func() {
		...
		fwr = clservev3.NewBufferResponse(wr, -1)
	...
	Describe("reset behaviour", func() {
		It("should allow re-writing after reset", func() {
			rec := httptest.NewRecorder()
			resp = clservev3.NewBufferResponse(rec, -1)
		...
		It("should reset limit after reset", func() {
			rec := httptest.NewRecorder()
			resp = clservev3.NewBufferResponse(rec, 2)
			for range 3 {
				resp.Reset()
